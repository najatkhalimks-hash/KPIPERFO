...
interface Props { project?: Project | null; researcherId?: string; onClose: () => void; onSaved: () => void }

export default function ProjectModal({ project, researcherId, onClose, onSaved }: Props) {
  const [form, setForm] = useState({
    title: '', type: '', role: 'PI', status: 'submitted', funder: '',
    total_budget: '', um6p_share_pct: 100, um6p_budget: '',
    start_date: '', end_date: '', is_international: false, comment: '',
  })
  ...
  const calcBudget = (total: string, pct: number) => {
    ...
    if (!isNaN(t)) set('um6p_budget', String(Math.round(t * pct / 100)))
  }
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal-box max-w-xl">
        <div className="flex items-center justify-between p-6 border-b border-um6p-border">
          <h2 className="text-lg font-semibold text-um6p-navy">{project ? 'Modifier le projet' : 'Ajouter un projet'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className="input-field" value={form.type} onChange={(e) => set('type', e.target.value)}>
                ...
                {['ANR', 'H2020', 'Horizon Europe', 'Bilatéral', 'Contrat industriel', 'National', 'Interne UM6P', 'Autre'].map((t) => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Rôle</label>
              <select className="input-field" value={form.role} onChange={(e) => set('role', e.target.value)}>
                {['PI', 'Co-PI', 'Co-investigateur', 'Partenaire', 'Participant'].map((r) => <option key={r}>{r}</option>)}
              </select>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className="input-field" value={form.status} onChange={(e) => set('status', e.target.value)}>
                {['idea', 'submitted', 'obtained', 'active', 'completed', 'cancelled'].map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" className="input-field" value={form.total_budget}
            ...
            <div>
              ...
              <input type="number" min={0} max={100} className="input-field" value={form.um6p_share_pct}
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" className="w-4 h-4 accent-um6p-green" checked={form.is_international} onChange={(e) => set('is_international', e.target.checked)} />
            <span className="text-sm text-um6p-navy">Projet international</span>
          </label>
          <div>
